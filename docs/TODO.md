...
- [x] Quality scores pipeline — `ml/extraction/03_quality_scores.py`: Bayesian average on vote_average + vote_count (m=median~14, C=mean~5.90), normalized to [0,1]. Spot checks: Fight Club 0.83, Pulp Fiction 0.84, Dark Knight 0.84. 4.5 MB (2026-03-26)
...
- [x] Online scoring Phase 2 — `ml/scoring/user_profile.py` (weighted-avg profile vectors, SQLite cache), `ml/scoring/scoring.py` (10-signal cosine similarity, dynamic weights, ~8ms/300 candidates), `ml/scoring/mood_filter.py` (threshold fallback 0.3→0.0) (2026-03-26)
...
- [x] Discover personalized sort (Phase 4.2) — ML scoring re-ranks discover candidates when sort=Personalized. Mood filter + 10-signal scoring via `ml/scoring/`. Graceful degradation to popularity order (2026-03-27)
